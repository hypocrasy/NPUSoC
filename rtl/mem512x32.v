...
//       
//       Creation Date:  Tue Oct  5 21:40:03 2021
//       Version: 	r1p1
//
...
`ifdef POWER_PINS
module mem512x32 (VDDCE, VDDPE, VSSE, Q, CLK, CEN, WEN, A, D, EMA, EMAW, RET1N);
`else
...
`endif
  ...
  reg NOT_A0, NOT_D31, NOT_D30, NOT_D29, NOT_D28, NOT_D27, NOT_D26, NOT_D25, NOT_D24;
 ...
 wire  CLK_;
  ...
  buf B79(EMAW_[0], EMAW[0]);
  buf B80(EMAW_[1], EMAW[1]);
  buf B81(RET1N_, RET1N);

   `ifdef ARM_FAULT_MODELING
     mem512x32_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module mem512x32_error_injection (Q_out, Q_in, CLK, A, CEN, WEN);
   output [31:0] Q_out;
   input [31:0] Q_in;
   input CLK;
   input [8:0] A;
